        ...
        if dtype.is_integer():
            for suffix, min_val, max_val in converts:
                all_in_range = (
                    df.lazy()
                    .select(
                        is_zero=pl.col(col) == 0,
                        is_inside=((pl.col(col) >= min_val) & (pl.col(col) <= max_val)),
                    )
                    .select(
...
"""
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._base_schema: pl.Schema | None = None
...
class DtBrowser(Widget):  # pylint: disable=too-many-public-methods,too-many-instance-attributes
    ...
    BINDINGS = [
        ...
        ("x", "show_expression", "Compute Expressions..."),
    ...
    def __init__(
        ...
        source_file_or_table: pathlib.Path | pl.DataFrame,
    ):
        super().__init__()
        bt = (
        ...
        self._color_selector = ColumnSelector(
            ...
            title="Select columns to color by",
        )
    ...
    @on(ExpressionBox.ExpressionSubmitted)
    @work(exclusive=True)
    async def update_expressions(self, event: ExpressionBox.ExpressionSubmitted):
        (foot := self.query_one(TableFooter)).pending_action = "Computing"
        ctx = pl.SQLContext(frames={"dt": self._original_dt})
        try:
            ...
            if event.column_name not in self.all_columns:
                ...
                self.all_columns = self.all_columns + (event.column_name,)

            await self._expression_box.expression_added(event.column_name, event.value)
            await self._apply_filter()
            self.watch_cur_row()
        except Exception as e:
            self.notify(
                f"Failed to apply expression '{event.value}' due to: {e}",
                severity="error",
                timeout=10,
            )
        finally:
    ...
    async def _apply_filter(self):
        if not self.current_filter:
            ...
            try:
                ...
                if dt.is_empty():
                    self.notify(
                        f"No results found for filter: {query}",
                        severity="warning",
                        timeout=5,
                    )
                else:
    ...
    async def watch_active_search(self, goto: bool = True):
        ...
        try:
            ...
            self.query_one(FilterBox).query_failed(query)
            self.notify(
                f"Failed to run search due to: {e}  {repr(e)}",
                severity="error",
                timeout=10,
            )
            foot.search_pending = False
    ...
    def action_iter_search(self, forward: bool):
        ...
        self.active_search_idx = min(
            max(self.active_search_idx + (1 if forward else -1), 0),
            len(self.active_search_queue) - 1,
        )
    ...
    def action_toggle_bookmark(self):
        ...
        self.notify(
            "Bookmark added!" if did_add else "Bookmark removed",
    ...
    async def action_show_save(self):
        ...
        self.notify(
            f"Successfully wrote {num_rows:,} rows / {sizeof_fmt(size)} to {target}",
            severity="information",
            timeout=5,
        )
    ...
    async def watch_timestamp_columns(self):
        ...
        if self._ts_col_names:
            ...
            try:
                ...
                self.notify(
                    f"Failed to compute timestamp columns: {e}",
                    severity="warn",
                    timeout=5,
                )
            finally:
    ...
    async def action_show_filter(self):
        ...
        if existing := self.query(".toolbox"):
            existing.remove()
            return
        self._filter_box.is_goto = False
        await self.mount(self._filter_box, before=self.query_one(TableFooter))

    async def action_show_expression(self):
        if existing := self.query(".toolbox"):
            existing.remove()
            return
        await self.mount(self._expression_box, before=self.query_one(TableFooter))
        ...
        if not self.color_by:
            ...
            try:
                ...
                self._meta_dt = (
                    ...
                    .join(
                        self._original_meta.lazy().select([INDEX_COL, COLOR_COL]),
                        how="left",
                        on=INDEX_COL,
                    )
                    .collect_async()
                ...
                self.notify(
                    f"Failed to apply coloring due to: {e}",
                    severity="error",
                    timeout=10,
                )
            foot.pending_action = None
    ...
    def compose(self) -> ComposeResult:
        ...
        self._expression_box.data_bind(read_only_columns=DtBrowser.read_only_columns)
